...
public class TransactionController {
	...
	/**
	 * <p>Updates an existing Transaction</p>
	 * @param user a Transaction Entity to update
	 * @return an HTTP Response with Code 200; an HTTP Response with Code 500 if a problem occurred
	 */
	@PutMapping("/transaction")
	public ResponseEntity<HttpStatus> updateTransaction(@RequestBody Transaction transaction) {
		ResponseEntity<HttpStatus> Response = new ResponseEntity<>(HttpStatus.OK);
		
		try {
			boolean Result = Service.updateTransaction(transaction);
			if (Result == false)
				throw new Exception("Could not update Transaction");
				
			log.info("[PUT] /transaction - " + Response.getStatusCode());
		} catch (Exception e) {
			Response = new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
			log.error("[PUT] /transaction - " + Response.getStatusCode() + " (" + e + ")");
		}
	...
	public ResponseEntity<HttpStatus> deleteConnection(@RequestParam int transactionId) {
		ResponseEntity<HttpStatus> Response = new ResponseEntity<>(HttpStatus.OK);
		...
		try {
			...
			log.info("[DELETE] /transaction - " + Response.getStatusCode());
		} catch (Exception e) {
			Response = new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
			log.error("[DELETE] /transaction - " + Response.getStatusCode() + " (" + e + ")");
		}
