...
public class TransactionControllerTest {
	...
	public void testUpdateTransaction() throws Exception {
		...
		this.mockMvc.perform(put("/transaction")
			.contentType(PMBUtil.APPLICATION_JSON_UTF8)
			.content(Body)
		).andExpect(status().isOk());

		verify(TransactionService, Mockito.times(1)).updateTransaction(any(Transaction.class));
	}
	
	@Test
	public void testUpdateTransactionNonValid() throws Exception {
		String Body = "{\"sender\": 1, \"receiver\": 2, \"description\": \"No details.\", \"amount\": 19.99, \"dateTransaction\": \"2025-01-01 08:00:00\"}";
		
		// CASE#1 - Generic Exception Thrown
		when(TransactionService.updateTransaction(any(Transaction.class))).thenAnswer(invocation -> { 
			throw new Exception(); 
		});
		...
		when(TransactionService.updateTransaction(any(Transaction.class))).thenReturn(false);
	...
	public void testDeleteTransaction() throws Exception {
		when(TransactionService.deleteTransaction(any(int.class))).thenReturn(true);
		
		this.mockMvc.perform(delete("/transaction?transactionId=99"))
			.andExpect(status().isOk());

		verify(TransactionService, Mockito.times(1)).deleteTransaction(any(int.class));
	}
	
	@Test
	public void testDeleteTransactionNonValid() throws Exception {
		// CASE#1 - Couldn't delete
		when(TransactionService.deleteTransaction(any(int.class))).thenReturn(false);

		this.mockMvc.perform(delete("/transaction?transactionId=1"))
			.andExpect(status().isInternalServerError());
		
		// CASE#2 - Wrong arguments
		when(TransactionService.deleteTransaction(any(int.class))).thenReturn(true);
		
		this.mockMvc.perform(delete("/transaction?transactionId=AAAA"))
		...
		// CASE#3 - Generic Exception Thrown
		when(TransactionService.deleteTransaction(any(int.class))).thenAnswer(invocation -> { 
			throw new Exception(); 
		});
		
		this.mockMvc.perform(delete("/transaction?transactionId=3"))
			.andExpect(status().isInternalServerError());
	}
}
